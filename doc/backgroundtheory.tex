...
measurement. The angular extent of an object in the sky provides us with another measurable quantity related to distances in the universe. The angular diameter distance is, for an object with a known size
...
$T_{\mu\nu}$, which describes the energy content of the universe. During the 1920s, by assuming the universe is isotropic, homogenous and expanding, the scientist's Alexander Friedmann and Georges Lemaître worked independently on finding a solution to equation \ref{eq:einstein}. In the 1930s Howard P.Robertson and Arthur Geoffrey Walker further explored the problem and proved that this is the only solution on a spacetime that is homogenous and isotropic.
...
prove beneficial for approximating and describing multiple phenomena. One of
...
perturbations grow over time. The small structure deviations at early times evolve through what is known as gravitational instability.
...
depend on $\vec{x}$. Therefore $\delta(\vec{x}, t)$ can then be expressed as a spatial and time-dependent expression on the form 
\begin{equation}
...
as a single force from one large body instead. The advantage with this approach is that it, instead of the calculations scaling as $\mathcal{O}(N)$ as for the regular pairwise summation,
...
target distribution is the likelihood or the posterior distribution. One of the
