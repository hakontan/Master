...
\end{equation}
When expanding this equation it is appropriate with a review of the terms. It is
...
\begin{equation}\label{eq:corr_no_stream}
    \xi^s_{\mathrm{vg}}(\vec{s},\mu)=\xi^r(r)+\frac{f}{3}\Delta_v(r)(1+\xi^r(r))+f\mu^2[\delta(r)-\Delta_v(r)](1+\xi^r(r)).
\end{equation}
...
model taking into account dispersion in the velocity of halos \cite{Achitouv_streaming}
\begin{equation}
    1+\xi^s(s,\mu)=\int_{-\infty}^\infty\frac{1+\xi^r(r)}{\sqrt{2\pi}\sigma_{v_{\parallel}(r)}}\mathrm{exp}\Big[-\frac{v_\parallel^2}{2\sigma_{v_\parallel}(r)}\Big]dv_\parallel.
\end{equation}